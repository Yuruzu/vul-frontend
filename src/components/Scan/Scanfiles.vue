        ...
        <el-table-column type="expand">
          <template slot-scope="props">
            <el-form label-position="left" inline class="demo-table-expand">
              <ul>
                <li  v-for="(item, index) in props.row.files_info" :key="index">{{item}}</li>
              </ul>
            </el-form>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    seachScans() {
      this.$http.post("http://192.168.32.126:8080/collectmessage/scanfileshistory", this.queryInfo)
      .then((res) => {
    ...
    scanFiles() {
      ...
      this.portdialogVisible = false
      this.$http.post("http://192.168.32.126:8080/collectmessage/scanfiles", this.fileform)
      .then((res) => {
    ...
    deleteScan(index, row) {
      ...
      this.$confirm("此操作将永久删除数据, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.$http.post("http://192.168.32.126:8080/collectmessage/scanfilesdelete", this.deleteScanData)
            .then((res) => {
              if (res.data.status == 'success') {
                this.seachScans();
                this.$message({
